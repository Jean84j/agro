<?php

namespace backend\widgets;

use common\models\shop\ActivePages;
use common\models\Posts;
use yii\base\Widget;
use yii\helpers\ArrayHelper;

class SubTopPosts extends Widget
{
    public function init()
    {

        parent::init();

    }

    public function run()
    {
        // 1. Получаем все посты (slug => id)
        $monthsAgoTimestamp = strtotime('-5 months');

        $allProducts = Posts::find()
            ->select(['id', 'slug', 'title'])
            ->where(['<', 'date_public', $monthsAgoTimestamp])
            ->orderBy(['date_updated' => SORT_ASC])
            ->indexBy('slug')
            ->asArray()
            ->all();

        // 2. Получаем все просмотры страниц с /post/
        $pages = ActivePages::find()
            ->select(['url_page', 'date_visit'])
            ->where(['like', 'url_page', '/post/'])
            ->asArray()
            ->all();

        // 3. Считаем просмотры по слагам
        $viewsCount = [];

        foreach ($pages as $page) {
            $url = $page['url_page'];
            $date = $page['date_visit'];

            if (str_contains($url, '/post/') !== false) {
                $parsedUrl = parse_url($url);
                $path = str_replace(['/en/', '/ru/'], '/', $parsedUrl['path']);
                $slug = trim(str_replace('/post/', '', $path), '/');

                if (!empty($slug)) {
                    if (isset($viewsCount[$slug])) {
                        $viewsCount[$slug]['count'] += 1;
                        if ($date > $viewsCount[$slug]['date']) {
                            $viewsCount[$slug]['date'] = $date;
                        }
                    } else {
                        $viewsCount[$slug] = [
                            'count' => 1,
                            'date' => $date,
                        ];
                    }
                }
            }
        }

        // 4. Находим post с нулевым или малым числом просмотров
        $lowViewed = [];
        $threshold = 10; // Меньше 3 просмотров считается "мало"

        foreach ($allProducts as $slug => $post) {
            $count = $viewsCount[$slug]['count'] ?? 0;

            if ($count < $threshold) {
                $lowViewed[$slug] = [
                    'slug' => $slug,
                    'name' => $post['title'],
                    'count' => $count,
                    'date' => $viewsCount[$slug]['date'] ?? null,
                ];
            }
        }

        // Сортируем по количеству просмотров (по возрастанию)
        ArrayHelper::multisort($lowViewed, ['count'], [SORT_ASC]);

        // 5. Берем первые 10
        $results = array_slice($lowViewed, 0, 10);

        // 6. Добавляем картинки
        foreach ($results as &$result) {
            $postData = Posts::find()
                ->alias('p')
                ->select([
                    'p.title as name',
                    'p.id',
                    'p.date_public',
                    'p.date_updated',
                    'p.webp_small',
                ])
                ->where(['p.slug' => $result['slug']])
                ->asArray()
                ->one();

            if ($postData) {

                $result['date_public'] = $postData['date_public'];
                $result['date_updated'] = $postData['date_updated'];

                $result['image'] = $postData['webp_small'];
            } else {
                $result['image'] = null;
            }
        }

        return $this->render('recent-activity', [
            'results' => $results,
            'catalog' => 'posts',
            'catalogImage' => 'posts',
            'titleWidget' => 'Статті без переглядів або з дуже малими переглядами',
        ]);
    }
}